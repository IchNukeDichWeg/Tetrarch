...
| 11 | Which piece the §8.3 multi-check bonus is credited to | **ASSUMPTION: the piece that moved.** The rule names one checker -- "with a queen" against "with any other piece" -- so what is counted is the kings the MOVED piece attacks, not every king in check afterwards. A discovered check by a third piece is not covered by the wording at any value, and neither is the rook of a castling move: only the king's destination is tested. Both are left uncredited rather than guessed at. A promoted queen counts as a queen, since the discount exists because a queen forks two lines without moving | 8.3 |
